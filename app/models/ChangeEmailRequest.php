...
class ChangeEmailRequest extends ApiModel
{
    use ClientHelper;
    ...
    {
        $activateUrl = $this->getDi()->getEnvUrl('clientUrl', sprintf('/activate-login/%s', $this->getCode()));
        ...
        $message = $mailer->createMessageFromView('activate_login', $vars)
            ->to($this->getEmail(), $client->getFirstName() . ' ' . $client->getLastName())
    ...
    {
        parent::initialize();
        $this->belongsTo('client_id', 'MrcHub\Models\Client', 'id', array('alias' => 'Client'));
    }
